...
- **Remaining gap: full mid-session secondmate endpoint liveness.** The `fm_backend_agent_alive`-driven respawn sweep (`bin/fm-bootstrap.sh`, see "Agent liveness probe reuses the husk classifier" above) still runs only at session start.
  The watcher now separately alarms when a recorded secondmate home's watcher beacon is confirmed stale while a live child can still produce, or has just produced, a wake; [architecture.md](architecture.md) owns that supervision-stall contract.
  This does not prove that an otherwise idle secondmate endpoint is alive, so detecting and respawning that endpoint during a session remains separate work.
