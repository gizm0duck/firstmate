...
8. Ordinary wake: when the background task completes with `signal:`, `stale:`, `check:`, `heartbeat`, `supervision:`, or `deadline:`, drain queued wakes, then start exactly one fresh background task before running other fleet commands to handle the wake.
...
10. The turn-end guard remains the final backstop and is not replaced by this command gate.
    Its harness-specific behavior is documented in [`turnend-guard.md`](../turnend-guard.md).
