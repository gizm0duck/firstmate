...
On that same slow-check cadence, watcher-side no-mistakes supervision wakes the owning home once when an attributed `running` or `fixing` step exceeds `FM_NM_STALL_ACTIVE_SECS` without a step transition or crew status write (default 900 seconds), when an `awaiting_approval` or `fix_review` gate exceeds `FM_NM_STALL_PARKED_SECS` (default 480 seconds), or when the run advances while the crew status file does not.
