...
On that same slow-check cadence, watcher-side no-mistakes supervision wakes the owning home once when an attributed `running` or `fixing` step exceeds `FM_NM_STALL_ACTIVE_SECS` without a step transition or crew status write, when an `awaiting_approval` or `fix_review` gate exceeds `FM_NM_STALL_PARKED_SECS`, or when the run advances while the crew status file does not.
[`docs/configuration.md`](configuration.md) owns the stall-threshold defaults and overrides.
The wake line names the run, step, and task, and detector markers stay in that task's owning `state/` home, so shared code roots cannot misroute the escalation.
