...
Secondmate panes are intentionally exempt from ordinary stale-pane wakes, so the watcher separately probes each recorded secondmate home and alarms only when its own beacon is confirmed stale while a live child can still produce - or has just produced - a wake; `bin/fm-watch.sh` owns that probe and its routed-work deadline backstop.
...
The shared turn-end guard remains the final backstop for all five harness protocols.
Its Codex-specific bounded reassertion after a returned checkpoint is documented in [turnend-guard.md](turnend-guard.md).
