...
FM_SECONDMATE_SUPERVISION_GRACE=600   # seconds before a recorded secondmate home's watcher beacon is stale enough to trigger the child-wake supervision probe
FM_SECONDMATE_SUPERVISION_CONFIRM_SECS=5   # seconds between first and confirming stale-beacon samples for the secondmate supervision probe; 0 skips the confirmation delay
FM_SECONDMATE_DEADLINE_SECS=900   # seconds after a marked routed secondmate send before an unchanged secondmate status wakes the parent again; invalid or zero values fall back to 900
