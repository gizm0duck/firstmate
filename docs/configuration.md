...
Compatible means the shared bootstrap probe accepts `tasks-axi --version` as 0.2.2 or newer, `tasks-axi show --help` exposes `--full` for lossless reads, `tasks-axi update --help` exposes lossless `--body-file` replacement plus `--archive-body`, and `tasks-axi mv --help` exposes `[<id>...]` for atomic multi-ID handoffs.
...
The compatible CLI has no `update --append` contract: inspect with `tasks-axi show <id> --full`, then replace the considered body with `tasks-axi update <id> --body-file <path>`, adding `--archive-body` when the superseded body should remain recoverable.
