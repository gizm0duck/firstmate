...
test_stall_snapshot_preserves_attributed_step_and_gate_status() {
  ...
  FM_FAKE_AXI_STATUS="$(run_parked_scalar_gate_running fm/feat-stall-snapshot)"
  out=$(run_stall_snapshot "$d" stall)
  assert_contains "$out" $'01RUN\treview\tawaiting_approval\t0' "scalar parked gate defaults to awaiting approval"
