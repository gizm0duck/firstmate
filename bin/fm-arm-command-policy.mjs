...
function xModePathAllowed(value, root, home) {
  if (value === "config/x-mode.env" || value === "./config/x-mode.env") return true;
  if (!path.isAbsolute(value)) return false;
  const normalized = path.normalize(value);
  return normalized === path.join(path.normalize(root), "config/x-mode.env")
    || normalized === path.join(path.normalize(home), "config/x-mode.env");
}
...
function setupKind(info, context) {
  ...
  if ((values[0] === "source" || values[0] === ".") && values.length === 2 && xModePathAllowed(values[1], context.root, context.home)) return "source";
  if (values[0] === "[" && values[1] === "-f" && values[3] === "]" && values.length === 4 && xModePathAllowed(values[2], context.root, context.home)) return "test-source";
  return "";
}
